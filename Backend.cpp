...
void Backend::addSink(string &pipeline, int xPos, int yPos, int width, int height) {
...
void Backend::generateYaml(QString userInputType, QString userInputFile, QString userModel) {
    string config;
    string title = "Custom";
    string width = "1280";
    string height = "720";
...
string Backend::getPipelineString() {
...
Q_INVOKABLE void Backend::increase_easter_egg_click_cnt() {
...
Q_INVOKABLE quint32 Backend::get_easter_egg_click_cnt() {
    return easter_egg_click_cnt;
}

Q_INVOKABLE QString Backend::get_random_content() {
    int rand_idx = std::rand() % (sizeof(CONTENT)/sizeof(CONTENT[0]));
    return QString::fromStdString(CONTENT[rand_idx]);
}

Q_INVOKABLE QString Backend::ip_addr() {
    return ip_addr_p;
}

void Backend::set_ip_addr(QString ip_addr_n) {
    ip_addr_p = sdk_ver;
    if (ip_addr_n != "")
        ip_addr_p += " | " + ip_addr_n;
...
Q_INVOKABLE void Backend::leftMenuButtonPressed(int button, int x, int y, int width, int height, QObject* object) {
    string cl_pipeline;
    string od_pipeline;
    string ss_pipeline;
...
Q_INVOKABLE void Backend::startVideo(QObject* object, QString pipeline){
    GError *error = NULL;
...
Q_INVOKABLE void Backend::stopVideo(){
    if (gst_pipeline) {
...
Q_INVOKABLE void Backend::popupOkPressed(QString InputType, QString Input, QString Model, int x, int y, int width, int height, QObject* object) {
...
string Backend::replaceAll(string str, const string &remove, const string &insert) {
...
string Backend::trimString(string str)
{
...
void Backend::getCameraInfo(map<string, map<string,string>> &cameraInfo)
{
